...
func RemoteServices(ctx context.Context, cfg *httpcfg.HttpCfg, logger log.Logger, rootCancel context.CancelFunc) (
	...
	if cfg.WithDatadir {
		...
		if cc.Parlia != nil {
			...
			blobDb := kv2.New(kv.BlobDb, log.New()).Path(blobDbPath).MustOpen()
	...
	if cfg.WithDatadir {
		if cc != nil && cc.Bor != nil {
			...
			bscKv, err = kv2.New(kv.ConsensusDB, logger).Path(bscDbPath).Accede(true).Open(ctx)
			if err != nil {
				return nil, nil, nil, nil, nil, nil, nil, ff, nil, nil, err
			}
...
func (e *remoteConsensusEngine) init(db kv.RoDB, blockReader services.FullBlockReader, remoteKV remote.KVClient, logger log.Logger) error {
	...
	if cc.Aura != nil {
		...
		eng = parlia.NewRo(cc, bscKv, blockReader, logger)
	} else {
		eng = ethash.NewFaker()
	}
...
func (e *remoteConsensusEngine) Initialize(config *chain.Config, chain consensus.ChainHeaderReader, header *types.Header, state *state.IntraBlockState, syscall consensus.SysCallCustom, logger log.Logger, tracer *tracing.Hooks) error {
	if err := e.validateEngineReady(); err != nil {
		panic(err)
	}
...
func (e *remoteConsensusEngine) Finalize(_ *chain.Config, _ *types.Header, _ *state.IntraBlockState, _ types.Transactions, _ []*types.Header, _ types.Receipts, _ []*types.Withdrawal, _ consensus.ChainReader, _ consensus.SystemCall, skipReceiptsEval bool, _ consensus.SystemTxCall, _ int, _ log.Logger) (types.Transactions, types.Receipts, types.FlatRequests, error) {
