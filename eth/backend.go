...
func New(ctx context.Context, stack *node.Node, config *ethconfig.Config, logger log.Logger) (*Ethereum, error) {
	...
	blockReader, blockWriter, allSnapshots, allBorSnapshots, bridgeStore, heimdallStore, allBscSnapshots, agg, err := setUpBlockReader(ctx, rawChainDB, config.Dirs, config, chainConfig, logger)
	if err != nil {
		return nil, err
	}
	...
	blockRetire := freezeblocks.NewBlockRetire(1, dirs, blockReader, blockWriter, backend.chainDB, blobStore, heimdallStore, bridgeStore, backend.chainConfig, config, backend.notifications.Events, blockSnapBuildSema, logger)
	...
	if config.PolygonSyncStage {
		...
		backend.syncStages = stages2.NewDefaultStages(backend.sentryCtx, backend.chainDB, snapDb, blobStore, p2pConfig, config, backend.sentriesClient, backend.notifications, backend.downloaderClient,
			blockReader, blockRetire, backend.agg, backend.silkworm, backend.forkValidator, backend.engine, heimdallClient, heimdallStore, bridgeStore, recents, signatures, logger)
	...
	checkStateRoot := true
	pipelineStages := stages2.NewPipelineStages(ctx, backend.chainDB, blobStore, config, p2pConfig, backend.sentriesClient, backend.notifications, backend.downloaderClient, blockReader, blockRetire, backend.agg, backend.silkworm, backend.forkValidator, backend.engine, logger, checkStateRoot)
	...
	if config.PolygonSync {
		...
		backend.polygonDownloadSync = stagedsync.New(backend.config.Sync, stagedsync.DownloadSyncStages(
			backend.sentryCtx, stagedsync.StageSnapshotsCfg(
				...
				blockReader, backend.notifications, backend.engine, backend.agg, false, false, false, backend.silkworm, config.Prune,
...
func setUpBlockReader(ctx context.Context, db kv.RwDB, dirs datadir.Dirs, snConfig *ethconfig.Config, chainConfig *chain.Config, logger log.Logger) (*freezeblocks.BlockReader, *blockio.BlockWriter, *freezeblocks.RoSnapshots, *heimdall.RoSnapshots, bridge.Store, heimdall.Store, *freezeblocks.BscRoSnapshots, *libstate.Aggregator, error) {
	...
	return blockReader, blockWriter, allSnapshots, allBorSnapshots, bridgeStore, heimdallStore, allBscSnapshots, agg, nil
}
